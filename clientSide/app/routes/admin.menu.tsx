...
export const action: ActionFunction = async ({ request }) => {
  ...
  if (!token || !menuId) return json({ status: "error" });

  if (intent === "delete") {
    try {
      await axios.delete(`http://localhost:5000/api/admin/groceries/${menuId}`, {
...
try {
  ...
  if (role !== "admin") {
    ...
    const res = await axios.get<Menu[]>("http://localhost:5000/api/admin/menu", {
      headers: {
                ...
                <table className="table-auto w-full border border-gray-200">
                  ...
                  <tbody>
                    {menus.map((menu, index) => (
                      <tr key={menu._id} className="text-center text-amber-950 hover:bg-gray-100">
                        <td className="py-3 px-2 border">{index + 1}</td>
                        <td className="py-3 px-2 border">{menu.customerName}</td>
                        <td className="py-3 px-2 border">{menu.address}</td>
                        <td className="py-3 px-2 border">{menu.hallType}</td>
                        <td className="py-3 px-2 border">{ menu.decoration === true ? "✅" :"❎"}</td>
                        <td className="py-3 px-2 border">{menu.totalCost}</td>
...
>
  Edit
                        ...
                        <td className="py-3 px-2 border">
                          ...
                          <FetcherForm method="post" action="/admin/menu">
