...
export const action: ActionFunction = async ({ request }) => {
  ...
  if (!token || !bookingId) return json({ status: "error" });
...
try {
  ...
  if (role !== "admin") {
    ...
    const res = await axios.get<Booking[]>("http://localhost:5000/api/admin/bookings", {
      headers: {
...
const { Form: FetcherForm } = fetcher;
  ...
  const bookings = useLoaderData<Booking[]>();
                ...
                <table className="table-auto w-full border border-gray-200">
                  ...
                  <tbody>
                    {bookings.map((booking, index) => (
                      <tr key={booking._id} className="text-center text-amber-950 hover:bg-gray-100">
                        <td className="py-3 px-2 border">{index + 1}</td>
                        <td className="py-3 px-2 border">{booking.name}</td>
                        <td className="py-3 px-2 border">{booking.contact}</td>
                        <td className="py-3 px-2 border">{new Date(booking.bookingDate).toLocaleDateString()}</td>
                        <td className="py-3 px-2 border">{ booking.hallType}</td>
                        <td className="py-3 px-2 border">{booking.notes}</td>
...
>
  Edit
                        ...
                        <td className="py-3 px-2 border">
                          ...
                          <FetcherForm method="post" action="/admin/booking">
